var card =
  ...
  '          <div class="clmn_9"><input type="text" class="id" value="" onfocus="this.select();" readonly/><button class="btn btn-mini clip_board"><i class="fas fa-clipboard"></i></button></div>' +
  "        </div>" +
  '        <div class="row">' +
  '          <div class="clmn_1">PW</div>' +
  '          <div class="clmn_9"><input type="password" class="pw" value="" onfocus="this.select();" readonly/><button class="btn btn-mini clip_board"><i class="fas fa-clipboard"></i></button></div>' +
  "        </div>" +
  '        <div class="card_note_area"><div class="card_note_label">NOTE</div><textarea class="note card_note" readonly></textarea></div>' +
  "      </div>";
...
$(function () {
  $(".add_btn").on("click", function () {
    ...
    $("#account_modal").find("#note").val("");
    $("#account_modal").find("#idx").val("");
    $("#account_modal").fadeIn();
  });
  $("#modal_ok").on("click", function () {
    if ($("#account_modal").find("#idx").val() == "") {
      new_card($("#name").val(), $("#id").val(), $("#pw").val(), $("#note").val());
    } else {
      upd_card($("#name").val(), $("#id").val(), $("#pw").val(), $("#note").val(), $("#idx").val());
    }
  });
...
function new_card(name, id, pw, note) {
  var $card = $(card);
  $card.css("display", "none");
  $card.find(".name").text(name);
  $card.find(".id").val(id);
  $card.find(".pw").val(pw);
  $card.find(".note").val(note);
  ...
  $card.find(".card_edit").on("click", function () {
    ...
    $("#account_modal").find("#note").val(tgt.find(".note").val());
...
function upd_card(name, id, pw, note, idx) {
  var tgt = "";
  $(".card").each(function (index, element) {
    if (index == idx) {
      tgt = $(element);
...
function import_data() {
  ...
  $("#import_ok").on("click", function () {
    ...
    datas.forEach((data) => {
      new_card(data[0], data[1], data[2], data[3]);
    });
    $(this).closest(".js-modal").fadeOut();
    return false;
  });
